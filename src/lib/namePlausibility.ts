 ...
 * bloque un enregistrement légitime (récupérable — l'agent le retape), un
 * faux nom accepté part sur une fiche transmise à une autorité. Le second
 * coûte infiniment plus cher, mais un filtre qui rejette la moitié des vrais
 * noms serait vite désactivé par les hôtels eux-mêmes.
 *
 * Dupliqué à l'identique dans `frontend/api/_lib/namePlausibility.ts` : les
 * fonctions serverless Vercel (`api/`) ne partagent pas le bundling avec
 * `src/`, et cette logique est trop petite pour justifier une dépendance
 * inter-dossiers fragile.
 */

const MAX_TOTAL_LENGTH = 60;

// Un « mot » (séparé par espace/tiret) plus long que ceci est structurellement
// improbable pour un nom humain, mais typique d'un bloc de texte d'étiquette
// happé sans espaces par l'OCR — c'est exactement la forme de l'incident
// d'origine (un seul bloc de 40 caractères, aucun séparateur).
const MAX_TOKEN_LENGTH = 30;

/*
 * Fragments bureaucratiques longs et distinctifs (FR/EN/PT) : jamais des mots
 * courts comme « DATE » ou « SEX », qui collisionnent avec de vrais noms/lieux
 * (Essex, Sussex...). `includes()` volontaire : à ce stade le texte capté n'a
 * généralement plus aucun espace.
 *
 * Toujours SANS ACCENT : `onlyLetters()` ci-dessous SUPPRIME les caractères
 * accentués plutôt que de les transformer (« RÉPUBLIQUE » → « RPUBLIQUE », pas
 * « REPUBLIQUE ») — un fragment accentué ne matcherait donc jamais rien.
 */
...
export function isSuspiciousName(value: string | null | undefined): boolean {
  ...
  // Lettres (accents compris), espaces, apostrophes, tirets, points (« J. »).
