  ...
  return (
    <div className="flex flex-col gap-2 border-b border-gray-100 py-3 last:border-0 sm:flex-row sm:items-center sm:justify-between">
      <div className="min-w-0">
        ...
        {row.updated_at && (
          <p className="text-xs text-gray-400">{t('aiCosts.updatedAt', { date: new Date(row.updated_at).toLocaleString('fr-FR') })}</p>
        )}
      </div>
      ...
      {editing ? (
        <div className="flex flex-wrap items-center gap-2">
          ...
          {confirming ? (
            <>
              <span className="text-xs text-gray-500">{t('aiCosts.confirmSave')}</span>
        ...
        <div className="flex items-center gap-4">
          <span className="font-mono text-sm text-gray-700">
            {formatUSD(row.input_price_per_mtok_usd, 4)} <span className="text-xs text-gray-400">/ Mtok in</span>
          </span>
          <span className="font-mono text-sm text-gray-700">
            {formatUSD(row.output_price_per_mtok_usd, 4)} <span className="text-xs text-gray-400">/ Mtok out</span>
          </span>
      ...
      {summary && (
        <>
          ...
          <div className="card p-5">
            ...
            <p className="mt-2 text-xs text-gray-400">{t('aiCosts.passportFallbackNote')}</p>
          </div>
          ...
          <div className="card p-5">
            ...
            <p className="text-xs text-gray-400 mb-3">{t('aiCosts.pricingHint')}</p>
            {(pricing ?? []).map((row) => (
